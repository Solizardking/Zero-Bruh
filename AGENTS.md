...
- **Skills (optional Solana host library)** — `~/skills/skills` when the bundled pack is not on the cwd walk
- **Agents** — `~/agents/agents/src` (`CLAWDBOT_AGENTS_DIR`) or override
...
The go-bot tree vendors a redistributable Robinhood Chain / Uniswap EVM skill pack
under `./skills` so **anyone** can clone and run discovery without private paths.
See [docs/RH_CRYPTO_AGENT_STACK.md](docs/RH_CRYPTO_AGENT_STACK.md).

Pack skills include: `rh-bonded-launch`, `rh-launchpad-v3`, `swap-integration`,
`swap-planner`, `viem-integration`, `copy-trade`, `dca-bot`, `index-bot`,
`liquidity-planner`, `lp-integration`, `deployer`, `pay-with-any-token`,
`pay-with-app`, `v4-hook-generator`, `v4-sdk-integration`, `v4-security-foundations`.

```bash
export CLAWDBOT_SKILLS_DIR="$(pwd)/skills"   # optional; defaults to ./skills when present
